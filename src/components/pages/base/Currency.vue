        ...
        <section class="content">
            ...
            <content-box :title="title" @open-modal="openModal">
                ...
                <template slot="pagination">
                    <p v-for="page in pageList">{{page}}</p>
    ...
    export default {
        ...
        created() {
            var run = async function () {
                let currency = await api.getCurrencyList()
                this.pushTableBody(currency.data.list)
                this.dataContent.data = this.tableContent
                this.dataContent.columns = this.tableColumns

                this.pageData = currency.data.paginator
                this.pageList = this.makePageList(this.pageData.page_num)
                console.log(currency.data.paginator)
            }
            run.call(this)
            this.loading = true
        },
